...
# Directories
WORKFLOW_APP_DIR = Path(__file__).parent / "workflow_app"
PAGES_DIR = WORKFLOW_APP_DIR / "pages"
PAGES_DIR.mkdir(parents=True, exist_ok=True)

# State files
TASKS_FILE = PAGES_DIR / "tasks.json"
...
def render_header():
    ...
    <div class="main-header">
        ...
        <p style="font-size: 0.9em; opacity: 0.9;">Generated workflows run in a separate Workflows Application</p>
    </div>
    """, unsafe_allow_html=True)
...
def main():
    ...
    registry = load_workflows_registry()
    if registry:
        st.subheader("📋 Generated Workflows")
        st.info(f"💡 You have {len(registry)} workflow(s) created. Launch the Workflows App to run them!")
        
        st.markdown("---")
        st.subheader("⚡ Launch Workflows Application")
        st.code("streamlit run workflows_app.py --server.port 8502", language="bash")
        st.markdown("---")
        ...
        # Page exists, test it
            ...
            if success:
                ...
                <div class="success-box">
                    ...
                    <p>✅ Your workflow is ready to run!</p>
                    <p><strong>To run your workflow:</strong></p>
                    <ol>
                        <li>Launch the <strong>Workflows Application</strong> (separate from this builder)</li>
                        <li>Run: <code>streamlit run workflows_app.py --server.port 8502</code></li>
                        <li>Your workflow will appear as a page in that application</li>
                    </ol>
                </div>
                """, unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        5. **Run** - Launch the Workflows App to use your workflows!
        """)
